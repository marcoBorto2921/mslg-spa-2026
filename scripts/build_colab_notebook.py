...
def build_cells() -> list:
    ...
    cells.append(
        code(
            ...
            "!pip install -q transformers==4.57.6 peft==0.18.1 sentencepiece==0.2.0 \\\n"
    ...
    cells.append(
        code(
            "# 3.2 - Patch config paths (point configs to /content/data_local)\n"
            "import os, sys, yaml\n"
            "\n"
            ...
            'print("Patched configs/baseline.yaml")\n'
            "\n"
    ...
    cells.append(
        code(
            "# 5.3 - Generate FORWARD back-translation data (SPA→MSLG synthetic)\n"
            "# Requires baseline checkpoints from 5.1 + 5.2.\n"
            "# Skip automatically if augmented_train.tsv already exists locally or on Drive.\n"
            "import os, shutil\n"
            "from pathlib import Path\n"
            "\n"
            "os.chdir(str(PROJECT_ROOT))\n"
            'aug_dst = LOCAL_DATA / "augmented_train.tsv"\n'
            "if not aug_dst.exists() and (DRIVE_DATA / aug_dst.name).exists():\n"
            "    shutil.copy2(DRIVE_DATA / aug_dst.name, aug_dst)\n"
            '    print("Copied augmented_train.tsv from Drive.")\n'
            "if aug_dst.exists():\n"
            '    print("augmented_train.tsv already exists — skipping generation.")\n'
            "else:\n"
            '    print("Generating forward BT data (SPA→MSLG)...")\n'
    ...
    cells.append(
        md(
            "### 5B — Baseline + back-translation · `baseline_bt.yaml` / `baseline_bt_s2m.yaml`\n"
        )
    )

    cells.append(
        code(
            "# 5.4 - Train MSLG2SPA — baseline + forward BT\n"
            "import os\n"
            "os.chdir(str(PROJECT_ROOT))\n"
            "\n"
    ...
    cells.append(
        code(
            "# 5.5 - Generate REVERSE back-translation data (MSLG→SPA synthetic)\n"
            "# Requires the stronger baseline_bt MSLG2SPA checkpoint from 5.4.\n"
            "# Skip automatically if augmented_train_reverse.tsv already exists.\n"
            "import os, shutil\n"
            "from pathlib import Path\n"
            "\n"
            "os.chdir(str(PROJECT_ROOT))\n"
            'rev_dst = LOCAL_DATA / "augmented_train_reverse.tsv"\n'
            "if not rev_dst.exists() and (DRIVE_DATA / rev_dst.name).exists():\n"
            "    shutil.copy2(DRIVE_DATA / rev_dst.name, rev_dst)\n"
            '    print("Copied augmented_train_reverse.tsv from Drive.")\n'
            "if rev_dst.exists():\n"
            '    print("augmented_train_reverse.tsv already exists — skipping generation.")\n'
            "else:\n"
            '    print("Generating reverse BT data (MSLG→SPA)...")\n'
            "    !python scripts/back_translate.py \\\n"
            "        --config configs/baseline.yaml \\\n"
    ...
    cells.append(
        code(
            "# 5.6 - Train SPA2MSLG — baseline + reverse BT (MSLG→SPA synthetic)\n"
            "import os\n"
            "os.chdir(str(PROJECT_ROOT))\n"
            "\n"
    ...
    cells.append(
        code(
            "# 6.3 - Submission B: best BT model per subtask\n"
            "# MSLG2SPA: baseline_bt/mslg2spa/final       (trained in 5.4)\n"
            "# SPA2MSLG: baseline_bt_s2m/spa2mslg/final   (trained in 5.6)\n"
    ...
    cells.append(
        code(
            ...
            "# MSLG2SPA: ensemble over baseline_bt/mslg2spa checkpoints\n"
            "# SPA2MSLG: ensemble over baseline_bt_s2m/spa2mslg checkpoints\n"
            "# Outputs: {TEAM_NAME}_baseline_bt_ensemble3_MSLG2SPA.txt\n"
            "#          {TEAM_NAME}_baseline_bt_ensemble3_SPA2MSLG.txt\n"
            "_n = N_CHECKPOINTS if 'N_CHECKPOINTS' in dir() else 3\n"
            "import os, yaml\n"
            "os.chdir(str(PROJECT_ROOT))\n"
            "with open('configs/baseline_bt.yaml') as f:\n"
            "    _bt = yaml.safe_load(f)\n"
            "_bt_base = _bt['training']['output_dir']\n"
            "with open('configs/baseline_bt_s2m.yaml') as f:\n"
            "    _s2m = yaml.safe_load(f)\n"
            "_s2m_base = _s2m['training']['output_dir']\n"
            'print(f"MSLG2SPA ckpt base : {_bt_base}")\n'
            'print(f"SPA2MSLG ckpt base : {_s2m_base}")\n'
            "\n"
            "!python scripts/ensemble_predict.py --config configs/baseline_bt.yaml \\\n"
            "    --subtask mslg2spa \\\n"
            "    --checkpoint_dir {_bt_base}/mslg2spa \\\n"
            "    --team {TEAM_NAME} --solution baseline_bt_ensemble{_n} \\\n"
            "    --n_checkpoints {_n}\n"
            "!python scripts/ensemble_predict.py --config configs/baseline_bt_s2m.yaml \\\n"
            "    --subtask spa2mslg \\\n"
            "    --checkpoint_dir {_s2m_base}/spa2mslg \\\n"
            "    --team {TEAM_NAME} --solution baseline_bt_ensemble{_n} \\\n"
            "    --n_checkpoints {_n}\n"
        )
    )
